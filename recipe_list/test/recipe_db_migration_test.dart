...
void main() {
  ...
  group('recipe_db migration v11 → v12', () {
    ...
    test(
      'upgrade adds user_profile + recipe_creator_cache without '
      'touching favorites / recipes',
      () async {
        sqfliteFfiInit();
        final factory = databaseFactoryFfi;
        ...
        final db = await factory.openDatabase(
          ...
          options: OpenDatabaseOptions(
            ...
            onCreate: (db, _) async {
              ...
              await db.execute(
                'DROP TABLE IF EXISTS recipe_creator_cache',
              );
            },
          ),
        );
    ...
    test('fresh install at v12 creates both new tables', () async {
      ...
      // Column shape sanity.
      final profileCols = await db.rawQuery(
