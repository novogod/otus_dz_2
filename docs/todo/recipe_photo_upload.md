...
- [x] Локальная проверка: `docker compose down -v && docker compose
...
- [x] В `local_user_portal/utils/backup-service.js` добавить
  `backupRecipePhotoFile(buffer, storageUrl)` (по аналогии с
  `backupAvatarFile` / `backupJobPhotoFile`).
- [x] В `local_user_portal/utils/storage-upload.js` (обе функции —
  ...
  ```
- [x] Если backup использует таблицу `backup_storage_files` с
...
- [x] Метод:
  ...
  ```
- [x] SQL: `UPDATE recipes SET i18n = jsonb_set(i18n,
  '{en,strMealThumb}', to_jsonb($2::text), true), content_hash = $3
  WHERE id = $1`.
- [x] Покрыть юнит-тестом в `tests/recipes.test.js`: insert через
...
- [x] Импорты: `multer`, `os`, `fs`, `path`, `crypto`,
  `uploadToStorage` (из `../utils/storage-upload.js`).
- [x] Создать `recipePhotoUpload = multer({ storage:
...
- [x] Вспомогательный `cleanupTempFile(path)` (одна функция, общая
...
- [x] Подменить `app.post('/recipes', async (req, res) => {…})` на
  `app.post('/recipes', recipePhotoUpload.single('photo'), async
  (req, res) => {…})`.
- [x] Дискриминатор: `req.file` присутствует ⇒ multipart-flow,
...
- [x] JSON-flow: без изменений.
- [x] Новые ошибки: `413 payload_too_large` (multer `LIMIT_FILE_SIZE`
  → перехватить в catch и вернуть честный код), `400
  unsupported_mime` (multer fileFilter).
- [x] Если шаг 5 упал, рецепт остаётся с `pending://upload` —
...
- [x] Применять только когда `Content-Type` начинается с
  ...
  ```
- [x] Тест-проброс: 6 multipart запросов за 60 секунд → 6-й
  получает 429.
...
- [x] Mock storage-api через `nock` или ручной `http`-перехват
  (зависит от чем уже пользуется тест-сьют).
- [x] Тест 1 «multipart upload персистит publicUrl»:
...
- [x] Тест 2 «multipart upload с упавшим storage оставляет
  pending placeholder»: мок storage-api 500, ответ 502, рецепт в
  БД c `strMealThumb='pending://upload'`.
- [x] Тест 3 «JSON-only flow без изменений»: проверка что старый
  путь по-прежнему возвращает 201.
- [x] Тест 4 «mime filter rejects pdf»: multipart с `application/pdf`
  → 400 + `unsupported_mime`.
- [x] Прогон: 16 / 2 (12 baseline + 4 новых, 2 echo-gate baseline-fail).
...
- [x] Файл `local_user_portal/jobs/cleanup-orphan-recipe-photos.js`:
  ```sql
  WITH refs AS (
  ...
  ```
- [x] Удалять найденные через
  `DELETE FROM storage.objects WHERE bucket_id='recipe-photos' AND name = ANY($1)`.
- [x] Расписание: один раз в неделю, через `setInterval` в
  `server.js` либо через системный cron в `Dockerfile`.
- [x] Env-флаг `RECIPE_PHOTOS_CLEANUP_DISABLED=1` для отключения
  без редеплоя.
- [x] Лог: `cleaned <N> orphan recipe photos`.
...
- [x] Добавить:
  ...
  path: ^1.9.0
  ```
- [x] iOS — в `recipe_list/ios/Runner/Info.plist`:
...
- [x] Android — `AndroidManifest.xml`: разрешения
...
- [x] Метод:
  ...
  ```
- [x] Вынести `_mealToJson(Recipe)` в приватный helper, чтобы и
  `createRecipe` (JSON-only), и `createRecipeWithPhoto` шли через
  один сериализатор.
- [x] Юнит-тест в `recipe_list/test/data/api/recipe_api_test.dart`
...
- [x] Удалить TextField «URL фотографии» (или оставить как fallback
  когда picker недоступен — например, в web-сборке).
- [x] Добавить виджет-превью + 2 кнопки:
...
- [x] HEIC → JPEG конверт + downscale делает единый
  `downscaleForUpload(XFile)` из чанка 11.5 — не дублируем
  вызовы `flutter_image_compress` по экрану.
- [x] Хранить выбранный `File` в `_pickedPhoto`, рендерить через
  `Image.file(_pickedPhoto)` 120×120 с borderRadius.
- [x] Валидация: `_pickedPhoto != null` ⇒ ok; иначе показать
...
- [x] `recipe_list/lib/utils/photo_downscaler.dart`:
  ```dart
...
- [x] Helper `String imgproxyUrl(String src, int w, int h)` в
  ...
  * для локальных тестов / TheMealDB-бэкенда возвращать `src`
    без обёртки.
- [x] Применить в `RecipeCard`, `RecipeDetailsPage` (как уже
  сделано на серверной стороне в `getImgproxyUrl`).
- [x] Документация: добавить пример в
...
- [x] `addRecipePhotoFromGallery` —  «Из галереи».
- [x] `addRecipePhotoFromCamera` —  «Сделать снимок».
- [x] `addRecipePhotoRequired` —  «Прикрепите фотографию».
- [x] `addRecipePhotoRemove` —  «Удалить фото» (tooltip
  крестика и пункт в bottom-sheet).
- [x] `addRecipePhotoSourceTitle` — заголовок bottom-sheetа
  ( «Откуда взять фото?»).
- [x] `addRecipePhotoErrorAccessDenied` — SnackBar при
  PlatformException «photo_access_denied».
- [x] `addRecipePhotoErrorTooLarge` — SnackBar при
  `photo_too_large` (фото не сжалось под 5 МБ).
- [x] `a11y.addRecipePhotoPicker` — Semantics-лейбл превью
  ( «Фото рецепта, нажмите чтобы перевыбрать»).
- [x] Удалить (или пометить deprecated) `addRecipePhoto` если
  поле URL убрано (оставить, если решаем web-fallback).
- [x] `dart run slang` → регенерировать
  `lib/i18n/strings*.g.dart`.
- [x] Дописать геттеры в `recipe_list/lib/i18n.dart`.
...
- [x] Обновить `docs/add-recipe-feature.md` §3.2 (форма) и §9
  (что вне рамок) — фото-URL заменено на ImagePicker, «загрузка
  файлом» больше не «вне рамок».
- [x] Обновить `docs/add-recipe-feature.md` §10 — добавить файлы
  `imgproxy.dart`, миграцию `20260429_create_recipe_photos_bucket.sql`.
- [x] В `docs/project_log.md` сверху новая запись «Recipe photo
  upload» с тем же форматом (date + bullet-list per repo).
- [x] В `docs/recipe-photo-upload.md` §4 (чек-лист) проставить
