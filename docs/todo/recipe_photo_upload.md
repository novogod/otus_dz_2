...
- [ ] `addRecipePhotoRemove` —  «Удалить фото» (tooltip
  крестика и пункт в bottom-sheet).
- [ ] `addRecipePhotoSourceTitle` — заголовок bottom-sheetа
  ( «Откуда взять фото?»).
- [ ] `addRecipePhotoErrorAccessDenied` — SnackBar при
  PlatformException «photo_access_denied».
- [ ] `addRecipePhotoErrorTooLarge` — SnackBar при
  `photo_too_large` (фото не сжалось под 5 МБ).
- [ ] `a11y.addRecipePhotoPicker` — Semantics-лейбл превью
